...
export default function partB(): void {
  ...
  for (let antenna in antennas) {
    for (let i = 0; i < antennas[antenna].length - 1; i++) {
      for (let j = i + 1; j < antennas[antenna].length; j++) {
        ...
        while (true) {
          let antiNode1 = {
            x: antennas[antenna][i].x + counter * posDifferences.x,
            y: antennas[antenna][i].y + counter * posDifferences.y,
          };
          counter++;
        ...
        while (true) {
          let antiNode2 = {
            x: antennas[antenna][j].x - counter * posDifferences.x,
            y: antennas[antenna][j].y - counter * posDifferences.y,
          };
          counter++;
  ...
  console.log(`Part B: ${antinodes.size}`);
}
